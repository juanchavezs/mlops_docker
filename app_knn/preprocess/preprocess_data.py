...
class DataPreprocessor(BaseEstimator, TransformerMixin):
    ...
    def feature_generation(self, data: pd.DataFrame) -> pd.DataFrame:     
        ...
        logger.addHandler(file_handler)    
        try:
            logger.info("Generating and adding features to the DataFrame...")
            ...
            data.drop(['ID','Year_Birth', 'Z_CostContact', 'Z_Revenue', 'Dt_Customer', 'Marital_Status'], axis=1, inplace=True)
        
            # Apply one-hot encoding for remaining categorical features
            data = pd.get_dummies(data)

            logger.info("Feature generation and addition completed.")
            return data
        
        except Exception as e:
            # Logging errors during feature generation
            logger.error(f"An error occurred during feature generation: {str(e)}")
            return data
